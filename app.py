...
# --- EKRAN TARCZY (PUNKTACJA) ---
    ...
    if st.button(T[lang]["finish"], type="primary", use_container_width=True):
        ...
        if sukces:
            st.success("✅ Wyniki zostały pomyślnie zapisane w Arkuszu Google!")
        else:
            st.error("❌ Błąd zapisu do Arkusza. Sprawdź nazwę pliku i uprawnienia.")
